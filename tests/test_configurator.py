...
# Configurator.
from dls_servbase_lib.configurators.configurators import Configurators
...
class TestGoodConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        # ---------------------------------------------------
        environ = {
            Envvar.DLS_BILLY_CONFIGFILE: "good_config1.yaml",
        }
...
class TestBadConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        BadConfigurationEnvTester(
            "RuntimeError", "DLS_BILLY_CONFIGFILE is not set"
        ).main(
            constants,
            {},
            output_directory,
        ...
        BadConfigurationEnvTester("RuntimeError", "does_not_exist.yaml").main(
            constants,
            {
                Envvar.DLS_BILLY_CONFIGFILE: "does_not_exist.yaml",
            },
            output_directory,
        )
...
class GoodConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        dls_servbase_configurator = Configurators().build_object(specification)

        # Perform symbol replacement.
        dls_servbase_configurator.substitute({"output_directory": output_directory})

        # Remove top level key of this name.
        dls_servbase_configurator.remove(["key_to_remove"])

        # Get the current dict representation of the configuration.
        current = await dls_servbase_configurator.load()
        ...
        value = dls_servbase_configurator.require("multi.level1.level2")
        assert value == "value_at_level2"

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = dls_servbase_configurator.require("multi.level1.level3")

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = dls_servbase_configurator.require("multi.level1.level2.level3")

        # Substitute in token.
        value = dls_servbase_configurator.resolve(
...
class BadConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        try:
            ...
            await dls_servbase_configurator.load()

            logger.info(
                describe("loaded yaml", dls_servbase_configurator.get_current())
            )
        except Exception as exception:
